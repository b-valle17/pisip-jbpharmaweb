...
public class EnsayoLaboratorioController {
	...
	public String listar(Model model, HttpSession session) {
		...
		model.addAttribute("puedeEditar",
				ensayos.stream().collect(Collectors.toMap(EnsayoLaboratorioResponseDto::getIdEnsayo,
	...
	public String nuevo(Model model, HttpSession session) {
		...
		if (dto.getFechaEnsayo() == null) {
		  ...
		  // Todo ensayo nuevo inicia obligatoriamente como REGISTRADO
	...
	@PostMapping("/guardar")
	public String guardar(@ModelAttribute("ensayo") EnsayoLaboratorioRequestDto dto, RedirectAttributes ra,
			HttpSession session) {
		dto.setIdEnsayo(null);
		dto.setCodigoEnsayo(null);
		dto.setResponsable(nombreUsuario(session));
	    // No aceptar el estado enviado desde el formulario
	    dto.setEstado("REGISTRADO");
	...
	private void asignarProductoDesdeOrden(EnsayoLaboratorioRequestDto dto) {
		OrdenProduccionResponseDto orden = listarOrdenes().stream().filter(o -> o.getIdOrden().equals(dto.getIdOrden()))
				.findFirst().orElseThrow(() -> new IllegalArgumentException("La orden seleccionada no existe."));
	...
	private void cargarRelaciones(Model model) {
		...
		Map<Integer, ProductoResponseDto> productosPorId = productos.stream().filter(p -> p.getIdProducto() != null)
				.collect(Collectors.toMap(ProductoResponseDto::getIdProducto, Function.identity(), (a, b) -> a));
		...
		model.addAttribute("nombresProductos",
				productosPorId.values().stream()
						.collect(Collectors.toMap(ProductoResponseDto::getIdProducto,
	...
	private String nombreUsuario(HttpSession s) {
		...
		if (n == null || n.toString().isBlank())
			throw new IllegalStateException("Sesión de usuario no válida.");
	...
	private String extraerMensaje(Exception ex) {
		...
		while (causa != null) {
			...
			if (causa instanceof WebClientResponseException webException) {

				String cuerpo = webException.getResponseBodyAsString();
